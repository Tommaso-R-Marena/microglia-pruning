...
class TestMicrogliaAgent:
    ...
    def test_forward_shape(self):
        ...
        # Create dummy input: (batch_size, 6*num_heads)
        batch_size = 4
        stats = torch.randn(batch_size, 6 * 32)
    ...
    def test_layer_positional_encoding_changes_masks(self):
        """Different layer indices should produce different masks for same stats."""
        agent = MicrogliaAgent(hidden_dim=64, num_heads=8, num_layers=12, layer_idx=0)
        stats = torch.randn(2, 6 * 8)

        masks_l0 = agent(stats, layer_idx=0)
        masks_l11 = agent(stats, layer_idx=11)

        assert not torch.allclose(masks_l0, masks_l11)
    ...
    def test_gradient_flow(self):
        ...
        stats = torch.randn(4, 6 * 32, requires_grad=True)
        masks = agent(stats)
